...
import static protocol.GitClient.HASH_BYTES_LENGTH;
import static protocol.GitClient.HEX;
...
public class PackParser {
    ...
    public List<PackObject> parse() throws IOException, DataFormatException {
        ...
        for (var index = 0; index < objectCount; ++index) {
            ...
            final PackObject object =
                    switch (type){case COMMIT : case TREE : case
                            BLOB : {final var content = inflate(header.size());
                        ...
                        case REF_DELTA: {
                            final var hashBytes = new byte[HASH_BYTES_LENGTH];
                            buffer.get(hashBytes);
                            final var baseHash = HEX.formatHex(hashBytes);
                            final var content = inflate(header.size());
                            final var contentBuffer = ByteBuffer.wrap(content);
                            @SuppressWarnings("unused")
                            final var baseObjectSize =
    ...
    public PackObjectHeader parseObjectHeader() {
        ...
        System.out.println(read & 0b00001111);
        final var type = git.pack.PackObjectType.valueOf((read & 0b01110000) >> 4);
        var size = read & 0b00001111;
